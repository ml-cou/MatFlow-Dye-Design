...
export default function ScalerEvaluationPage({ csvData }) {
  ...
  const run = async () => {
    ...
    setBusy(true);
    try {
      const { data } = await axios.post(`${API}/scale-evaluate/`, {
        dataset: rows,
                ...
                <thead>
                  <tr className="bg-gray-50">
                    {Object.keys(res.ranking[0]).map((k) => (
                      <th
                        key={k}
                        className="px-6 py-4 text-left text-sm font-bold text-gray-800 uppercase tracking-wider border-b border-gray-200"
                      >
                        {k}
                      </th>
                    ))}
              ...
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="relative group">
                    <button className="flex items-center text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 px-3 py-2 rounded-lg transition-colors">
                      <svg
                        className="w-5 h-5 mr-1"
                        fill="none"
                        stroke="currentColor"
                    ...
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-10">
                      <div className="p-2">
                        <div className="text-xs font-medium text-gray-500 px-3 py-1 mb-1">Individual Charts</div>
                        <button
                          onClick={() =>
                            downloadImage(
                  ...
                  <thead>
                    <tr className="bg-gray-50">
                      {Object.keys(res.scaled_preview[0]).map((k) => (
                        <th
                          key={k}
                          className="px-6 py-4 text-left text-sm font-bold text-gray-800 uppercase tracking-wider border-b border-gray-200 whitespace-nowrap"
                        >
                          {k}
                        </th>
                      ))}
