...
const GraphMLWorkflow = () => {
  ...
  const handleModelTrain = async () => {
    try {
      ...
      const fileProcessResponse = await fetch(
      ...
      if (!fileProcessResponse.ok) {
        throw new Error('File processing failed');
      }

      const processedData = await fileProcessResponse.json();
      ...
      const dataProcessResponse = await fetch(
        `${import.meta.env.VITE_APP_API_GRAPH}/process-data/`,
        {
          method: 'POST',
          body: dataProcessFormData
        }
      );

      if (!dataProcessResponse.ok) {
        throw new Error('Graph data processing failed');
      }

      const dataProcessJson = await dataProcessResponse.json();
