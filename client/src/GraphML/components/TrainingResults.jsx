...
const TrainingResultsView = ({ results }) => {
  ...
  const getTraceColor = (traceName, defaultColor) => {
    ...
    if (lowerName.includes('train')) return '#3b82f6'; // Blue
  ...
  const getMetricIcon = (key) => {
    switch(key) {
      case 'r2': return <TrendingUp className="w-4 h-4 text-blue-600" />;
      case 'rmse':
    ...
    <div className="mt-8 bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden transition-all hover:shadow-lg">
      <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-white">
        <h3 className="text-lg font-bold text-gray-800 flex items-center">
          <BarChart2 className="w-5 h-5 mr-3 text-blue-600" />
          Training Results
        </h3>
      </div>
        ...
        <div className="p-6 border-b border-gray-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(finalTestMetrics).map(([key, val]) => (
              <div key={key} className="bg-gradient-to-br from-blue-50 to-blue-100/50 p-4 rounded-lg border border-blue-200 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-center mb-2">
                  {getMetricIcon(key)}
                  <p className="text-xs font-medium text-blue-700 uppercase tracking-wider ml-1">{key}</p>
                </div>
